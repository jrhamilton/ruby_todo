...
def add_task(list_choice)
  ...
  while !Date.parse(due_date_var)
    puts "Invlalid Date format. Please input another Date correctly, fool!"
    due_date_var = gets.chomp
  end
  due_date = Date.parse(due_date_var)
  new_task = Task.new(user_description, priority, due_date)
...
def done_menu(list_choice, task_index)
  puts "\n"
  @lists[list_choice].tasks[task_index].mark_done
...
def edit_task_menu(list_choice)
  ...
  puts "\n"
  puts "To edit: select 'done' to mark as done,'desc' to give the task a new Description or 'delete' to delete a task."
  edit_by = gets.chomp
  case edit_by
  when 'done'
    puts "\n"
